...
if ($_GET['id'] != "") {
    ...
    echo "<div class='row bg-faded text-center'>
					<div class='col-xs-12'>
						<h3 class='m-b-0 p-b-md p-t-md'><a href='./" . $usern . "'>$" . $usern . "</a> is stalking:</h3>
					</div>
				</div>
				";
    ...
    if (mysqli_num_rows($follows) > 0) {
        ...
        echo "<h5><i>$" . $usern . " is stalking no one!</i><br>";
    }
    mysqli_close($con);
} else {
    echo "<div class='alert alert-danger'>Sorry, invalid query.</div>";
    echo "<a href='" . header("Location: " . $_SERVER['HTTP_REFERER']) . "' style='width:300px;' class='btn btn-info'>Back</a>";
}
